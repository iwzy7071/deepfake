...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, is_to_tensor=False, out_mode='torch'):
    ...
    if out_mode in 'torch':
        return im_to_torch(im_patch) / 255.0 if is_to_tensor else im_to_torch(im_patch)
    else:
        return im_patch
