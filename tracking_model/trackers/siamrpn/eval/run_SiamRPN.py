...
from trackers.siamrpn.eval.utils import get_subwindow_tracking
...
def SiamRPN_init(im, target_pos, target_sz, net, device):
    state = dict()
    p = TrackerConfig()
    p.update(net.cfg)
    ...
    z_crop = get_subwindow_tracking(im, target_pos, p.exemplar_size, s_z, avg_chans, p.to_tensor)
...
def SiamRPN_track(state, im, device):
    ...
    x_crop = Variable(get_subwindow_tracking(im, target_pos, p.instance_size, round(s_x), avg_chans, p.to_tensor).unsqueeze(0))
