    ...
    for fake_name, real_name in tqdm(fake2real):
        ...
        for box_index, boxes in enumerate(json.load(open(real_json_path))):
            ...
            for face_index, (real_image, fake_image, box) in enumerate(zip(real_images, fake_images, boxes)):
                ...
                cv2.imwrite(join(current_save_mask_dir, '{}_{}_.png'.format(box_index, face_index)), mask)
                cv2.imwrite(join(current_save_real_dir, '{}_{}_.png'.format(box_index, face_index)), real_face)
                cv2.imwrite(join(current_save_fake_dir, '{}_{}_.png'.format(box_index, face_index)), fake_face)
